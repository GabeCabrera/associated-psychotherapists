            ...
            <Hero>
                ...
                >
                    ...
                    >
                        ...
                        <Container sx={{ display: 'flex', flexDirection: 'column', alignItems: 'left' }}>
                            <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'baseline', mb: 1 }}>
                                <h4>EMERGENCY:</h4>  <h4 style={{ color: 'red'}} >&nbsp; 911 </h4>
                            </Box>

                            {resourceData.map((data) => (
                                <Box key={data.id} sx={{ display: 'flex', flexDirection: 'row', alignItems: 'baseline', mb: 1, textWrap: 'noWrap' }}>
                                    <h4>{data.title}:</h4>  <h4 style={{ color: '#6DA2D6'}} >&nbsp;{data.phoneNumber ? data.phoneNumber : ''} {data.website ? <a style={{ color: '#424242', fontWeight: 'normal', textDecoration: 'underline' }} href={'https://www.' + data.website}>{data.website}</a> : ''}</h4>
                                </Box>
                            ))}
                        </Container>
                    </Card>
                    <Typography variant="subtitle1" component="div" gutterBottom sx={{ fontFamily: 'Roboto', fontWeight: 300, fontSize: '0.8rem', mt: 3, maxWidth: '75%' }}>
